...
DROP POLICY IF EXISTS "Organizers can manage quiz state" ON quiz_state;
CREATE POLICY "Organizers can manage quiz state"
  ON quiz_state FOR ALL
  TO authenticated
  USING (
    quiz_id IN (
      SELECT id FROM quizzes WHERE organizer_id = auth.uid()
    )
  )
  WITH CHECK (
    quiz_id IN (
